            ...
            boolean autoPlayAnim)
    {
        ...
        if (hierarchy != null)
        {
            ...
            DraweeController controller = Fresco.newDraweeControllerBuilder().setImageRequest(
                    imageRequest).setAutoPlayAnimations(
                            autoPlayAnim).setOldController(getController()).setControllerListener(this).build();
            setController(controller);
        }
    }

    /**
     * {@link SimpleDraweeView} 展示出来主要有两个方面。
     * 1. {@link com.facebook.drawee.interfaces.DraweeHierarchy} 包含了展示信息：圆角、默认图等等
